...
try:
    ...
    print(f"[ACRL] Error importing libraries: {e}")

# Conditionally import ac and acsys
try:
    import ac  # noqa: E402
    ...
    ac = None
    acsys = None
    print("[ACRL] Warning: 'ac' and 'acsys' modules are not available. This script must be run inside Assetto Corsa.")
...
def get_invalid(car: int = 0) -> bool:
    ...
    if ac is None or acsys is None:
        raise RuntimeError("The 'ac' module is not available. This function must be run inside Assetto Corsa.")
    return ac.getCarState(car, acsys.CS.LapInvalidated) == 1
